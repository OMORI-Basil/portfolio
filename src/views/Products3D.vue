      ...
      <div class="sub-section">
        ...
        <div class="grid-row">
          <div v-for="(img, i) in landscape" :key="'l'+i" class="img-wrap">
            <div class="img-box land" :style="{ backgroundImage: img.url, backgroundPosition: img.pos || 'center' }"></div>
      ...
      <div class="sub-section">
        ...
        <div class="grid-row portrait-row">
          <div v-for="(img, i) in portrait" :key="'p'+i" class="img-wrap">
            <div class="img-box port" :style="{ backgroundImage: img.url, backgroundPosition: img.pos || 'center' }"></div>
            <p class="img-label">{{ img.name }}</p>
          </div>
        </div>
      </div>
...
const portrait = [
  ...
  { url: 'url(/images/化妆品3_后期.png)', name: '化妆品_自然风格渲染' },
...
.back-btn {
  font-family: var(--font-family); font-size: 0.8125rem; color: var(--color-text-muted);
  text-decoration: none; display: inline-flex; align-items: center; justify-content: center;
  padding: 0.4rem 1.2rem; border: 1px solid rgba(255,255,255,0.15); border-radius: 999px;
  margin-bottom: 2rem; transition: all 0.2s; gap: 0.3rem;
}
